...
pub fn handle_client(mut stream: TcpStream, log: Logger) -> Result<(), Error> {
    ...
    info!(
        log,
        "[timekeeper] Sending a Sync message to slave..."
    );
    let time_on_sync_sending = time::now().to_timespec();
    info!(log, "[timekeeper] Marked {}.{} as timestamp on sending Sync message", time_on_sync_sending.sec, time_on_sync_sending.nsec);
    ...
    info!(log, "[timekeeper] Sending Follow-Up message with {}.{} as timestamp in it", time_on_sync_sending.sec, time_on_sync_sending.nsec);
    ...
    info!(log, "[timekeeper] Marked {}.{} as timestamp on receiveing Delay Request", time_on_delay_request.sec, time_on_delay_request.nsec);
    ...
    info!(log, "[timekeeper] Sending Delay Response message with {}.{} as timestamp in it", time_on_delay_request.sec, time_on_delay_request.nsec);
